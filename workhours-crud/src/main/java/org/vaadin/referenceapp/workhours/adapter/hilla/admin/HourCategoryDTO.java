...
record HourCategoryDTO(
    ...
    HourCategory toEntity(LookupFunction<HourCategoryId, HourCategory> entityLookup) {
        var entity = Optional.ofNullable(id()).map(HourCategoryId::fromLong).flatMap(entityLookup::findById).orElseGet(HourCategory::new);
        entity.setName(name());
        return entity;
    }
}
